...
def main():
    ...
    reported_payoff, selected_intervals = read_program_output(output_file)
    ...
    check_payoff(expected_payoff, reported_payoff, selected_intervals)
    ...
    with open(input_file) as infile, open(output_file, 'w') as outfile:
        ...
        try:
            subprocess.check_call(
                [executable], stdin=infile, stdout=outfile,
...
def check_payoff(expected_payoff, reported_payoff, selected_intervals):
    """
    Check that the total payoff is consistent with the output intervals.
    """
    sum_payoff = sum(interval[2] for interval in selected_intervals)
    if sum_payoff != reported_payoff:
        error_and_exit('Output has inconsistent payoff: '
                       'reported {} but the sum of payoff is {}'
                       .format(reported_payoff, sum_payoff))
    if expected_payoff is not None and reported_payoff != expected_payoff:
        error_and_exit('Output payoff does not match expected payoff: '
                       'expected {} but the reported payoff is {}'
                       .format(expected_payoff, reported_payoff))
